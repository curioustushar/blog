...
author: Tushar Gupta
description: "A defended 2.5T-token mixture spec — seven capability lanes, Indic provenance tiers, OPUS floors, separate anneal preset, inventory supply checks, and proxy experiment results."
---

<div class="post-summary">

The mixture is the model. This specification defines **two presets** — a **main pretraining mixture** (web-heavy, scarce lanes small) and a **separate anneal mixture** (scarce lanes concentrated) — over a **2.5T-token** budget. Seven capability lanes sum to 100%, Indic splits across **verified / unverified / translated / synthetic** tiers, **OPUS selection** runs above protected floors (`Indic ≥ 12%`, `Agentic ≥ 2%`), and Tier-A data is **reserved** for the final cooldown. Every allocation is checked against the dataset inventory and treated as a **hypothesis** until proxy runs confirm it — **Phase 1 loader simulation confirmed**; full 3B @ 1B benchmark proxy scheduled.
...
| Screen (executed) | Char-LM smoke proxy + loader simulation — [scripts/mixture_proxy_experiment.py](https://github.com/curioustushar/blog/blob/master/scripts/mixture_proxy_experiment.py) |
...
| **P2** | Reasoning | 150B | **85B** AON inventory + **15.1M** AyAI rows cleaned ([cleaning blog]({{< relref "2026-07-23-data-cleaning-strategies.md" >}}): 22.2M→15.1M, ~68% survival, manifests generated) | ~65B @ 2.5T | Pipeline **proven** on 22M-row set; scale-up in progress |
